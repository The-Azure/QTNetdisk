...
Friend::Friend(QWidget *parent) : QWidget(parent)
{
    ...
    QBoxLayout *pMsgHBL = new QHBoxLayout;
    pMsgHBL->addWidget(m_pInputMs);
...
{/*
    QString str = "111";
    int *c = (int*)malloc(str.size()/4+1);
    memcpy(c, str.toStdString().c_str(), str.size());
    qDebug()<< (char*)c;*/


    QString strMsg = m_pInputMs->text();
    qDebug()<<"输入字节数为："<<strMsg.toUtf8().size()<<"改变之前是"<<strMsg.size()<<"内容"<<strMsg;
    if(!strMsg.isEmpty())
    {
